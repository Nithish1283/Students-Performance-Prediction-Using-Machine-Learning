...
def serviceproviderlogin(request):
    if request.method  == "POST":
        admin = request.POST.get('username', '').strip()
        password = request.POST.get('password', '').strip()
        print(f"DEBUG: Received username={repr(admin)}, password={repr(password)}", flush=True)
        if admin.lower() == "sprovider" and password.lower() == "sprovider":
            performance_ratio_model.objects.all().delete()
            search_ratio_model.objects.all().delete()
            return redirect('View_Remote_Users')
        else:
            return render(request, 'RUser/login.html', {'active_tab': 'provider', 'error': 'Invalid Service Provider username or password! (Note: Credentials are case-sensitive)'})

    return render(request, 'RUser/login.html', {'active_tab': 'provider'})
