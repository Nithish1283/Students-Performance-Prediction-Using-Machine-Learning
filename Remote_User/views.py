...
def login(request):
    ...
    if request.method == "POST" and 'submit1' in request.POST:
        ...
        try:
            ...
            enter = ClientRegister_Model.objects.get(username__iexact=username, password=password)
            request.session["userid"] = enter.id
            performance_ratio_model.objects.all().delete()
            search_ratio_model.objects.all().delete()
            return redirect('Add_DataSet_Details')
        except:
            return render(request, 'RUser/login.html', {'error': 'Invalid Student username or password!'})
